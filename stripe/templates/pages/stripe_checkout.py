...
import frappe
from stripe.api import create_stripe_subscription
...
def get_api_key(doc, gateway_controller):
	publishable_key = frappe.db.get_value("Stripe Setting", gateway_controller, "publishable_key")
...
def get_header_image(doc, gateway_controller):
	header_image = frappe.db.get_value("Stripe Setting", gateway_controller, "header_img")
...
def make_payment(stripe_token_id, data, reference_doctype=None, reference_docname=None):
	...
	if frappe.db.get_value(reference_doctype, reference_docname, 'is_a_subscription'):
		...
		data =  reference.create_stripe_subscription(gateway_controller, data)
	else:
		data =  frappe.get_doc("Stripe Setting", gateway_controller).create_request(data)
